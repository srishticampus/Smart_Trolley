...
void loop() {
  ...
  if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial()) {
    ...
    if ((content.substring(1) == "43 A8 2A C5") && (laysFlag==0)) {
      laysCount=laysCount+1;
      Serial.println(laysCount);
      if (Firebase.RTDB.setString(&fbdo, "/lays",String(laysCount))){
    ...
    else if ((content.substring(1) == "43 A8 2A C5") && (laysFlag==1)) {
      laysCount=laysCount-1;
      Serial.println(laysCount);
      if (Firebase.RTDB.setString(&fbdo, "/lays",String(laysCount))){
          Serial.println("lays PASSED"); 
    ...
    else if ((content.substring(1) == "03 EF 43 F6") && (biscutFlag==0)) {
      biscutCount=biscutCount+1;
      Serial.println(biscutCount); 
      if (Firebase.RTDB.setString(&fbdo, "/biscut",String(biscutCount))){
          Serial.println("biscut PASSED"); 
    ...
    else if ((content.substring(1) == "03 EF 43 F6") && (biscutFlag==1)) {
      biscutCount=biscutCount-1;
      Serial.println(biscutCount);
      if (Firebase.RTDB.setString(&fbdo, "/biscut",String(biscutCount))){
          Serial.println("biscut PASSED"); 
    ...
    if (Firebase.RTDB.setString(&fbdo, "/total",String(totalAmount))){
